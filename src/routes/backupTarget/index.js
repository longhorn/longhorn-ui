...
const filterDataByField = (backupTargetData, field, value, booleanValue) => {
  if (!field) return backupTargetData
...
class BackupTarget extends React.Component {
  ...
  render() {
    ...
    const backupTargetFilterProps = {
      ...
      booleanFields: ['readOnly', 'default', 'available'],
      onSearch(filter) {
        ...
        if (filterField && (filterValue || typeof filterBooleanValue === 'boolean')) {
          ...
          if (['readOnly', 'default', 'available'].includes(filterField) && typeof filterBooleanValue === 'boolean') {
