...
function Backup({ backup, volume, setting, backingImage, loading, location, dispatch }) {
  ...
  const backupTargetName = queryString.parse(location.search).backupTargetName || ''
  ...
  const backups = backupTargetName ? backupData.filter((item) => item.backupTargetName === backupTargetName) : backupData
  sortBackups(backups)
